...
static void append_log(State &state, const std::string &text) {
    ...
    state.log->bottomline(state.log->lines());
    state.log->redraw();
    Fl::check();
}
...
static void rebuild_depot_list(State &state) {
    ...
    for (const auto &[id, depot] : state.depots) {
        ...
        label << id << "  •  " << depot.versions.size() << " versions  •  "
              << depot.dat_count << " dats  •  " << depot.blob_count << " blobs";
...
static void show_depot(State &state, const std::string &id) {
    ...
    detail << "Depot " << id << "\n"
           ...
           << "Select a version below to see its CRCs.";
...
static void import_index(State &state) {
    ...
    while (std::getline(in, line)) {
        ...
        version = filename.substr(first + 1, second - first - 1);
        crc = filename.substr(second + 1, third - second - 1);

        auto &[depot_id, depot] = *state.depots.try_emplace(id, DepotInfo{id}).first;
...
static void browse_existing_index(State &state) {
    std::ifstream test(value(state.index_file));
    if (test.good()) import_index(state);
...
static void index_cb(Fl_Widget *, void *data) {
    ...
    state.progress->maximum(3);
    state.progress->value(0);
    append_log(state, "Indexing dump...");

    int rc = 0;
    const std::string output = run_tool(state, "index --base " + quote_shell(base) +
        " --out " + quote_shell(value(state.index_file)), rc);
    state.progress->value(2);
    append_log(state, output);

    if (rc == 0) import_index(state);
    else fl_alert("Indexing failed. See the Activity tab for details.");

    state.progress->value(3);
...
static bool validate_target(State &state) {
    ...
    if (!fs::exists(value(state.index_file))) {
        fl_alert("The index file does not exist. Index the dump first.");
        return false;
    }
    return true;
}
...
static void download_cb(Fl_Widget *, void *data) {
    ...
    std::string args = "download --base " + quote_shell(value(state.mirror)) +
    ...
    const std::string output = run_tool(state, args, rc);
    append_log(state, output);
    state.download_button->activate();
    if (rc != 0) fl_alert("The download failed. See the Activity tab for details.");
    else fl_message("Download finished.");
}
...
static void extract_cb(Fl_Widget *, void *data) {
    ...
    if (rc != 0) fl_alert("Extraction failed. See the Activity tab for details.");
    else fl_message("Extraction finished.");
}

static void download_extract_cb(Fl_Widget *widget, void *data) {
    auto &state = *static_cast<State *>(data);
    if (!validate_target(state)) return;
    download_cb(widget, data);
...
static void use_latest_cb(Fl_Widget *, void *data) {
    auto &state = *static_cast<State *>(data);
    const std::string id = value(state.depot);
    const auto it = state.depots.find(id);
    if (it == state.depots.end() || it->second.versions.empty()) return;
    const auto latest = it->second.versions.rbegin()->first;
    state.version->value(latest.c_str());
    show_version(state, id, latest);
}

static void copy_crc_cb(Fl_Widget *, void *data) {
    auto &state = *static_cast<State *>(data);
...
int main(int argc, char **argv) {
    ...
    auto *title = new Fl_Box(24, 14, 650, 32, "Steam2 Dump Tool");
    title->labelfont(FL_BOLD); title->labelsize(24); title->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    auto *subtitle = new Fl_Box(24, 47, 850, 24,
        "Browse the Steam2 index, pick a depot/version, download it, then extract it.");
    subtitle->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE); subtitle->labelsize(14);
    auto *default_btn = new Fl_Button(930, 24, 210, 36, "Use German Mirror");
    header->end();

    auto *tabs = new Fl_Tabs(16, 96, 1148, 650);

    auto *setup = new Fl_Group(16, 120, 1148, 620, "1. Setup & Index");
    auto *setup_box = new Fl_Group(32, 144, 1116, 230);
    auto *mirror_label = new Fl_Box(48, 162, 150, 28, "Mirror URL");
    ...
    auto *browse_index = new Fl_Button(990, 190, 140, 34, "Index file...");
    auto *index_label = new Fl_Box(48, 238, 150, 28, "Index output");
    ...
    auto *index_help = new Fl_Box(48, 392, 1050, 40,
        "The index is large. This reads the compact date index files rather than crawling millions of directory entries.");
    index_help->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE | FL_ALIGN_WRAP); index_help->labelsize(13);
    state.index_button = new Fl_Return_Button(48, 448, 300, 48, "Build / Refresh Index");
    state.index_button->labelsize(17);
    state.progress = new Fl_Progress(48, 514, 1050, 22);
    state.progress->minimum(0); state.progress->maximum(3); state.progress->value(0);
    auto *jump = new Fl_Button(48, 568, 300, 44, "Go to Depot Browser");
    setup->end();

    auto *browser = new Fl_Group(16, 120, 1148, 620, "2. Depot Browser");
    auto *search_label = new Fl_Box(34, 145, 100, 28, "Search depot");
    ...
    auto *depot_caption = new Fl_Box(34, 680, 540, 28, "Select a depot. RESET marks depots with multiple CRCs.");
    ...
    auto *latest = new Fl_Button(950, 220, 180, 32, "Use Latest Version");
    ...
    auto *details_btn = new Fl_Button(610, 542, 250, 34, "Record CRC in Activity");
    state.details = new Fl_Multiline_Output(610, 582, 520, 98);
    state.details->textfont(FL_COURIER); state.details->textsize(12);
    right->end();
    browser->end();

    auto *actions = new Fl_Group(16, 120, 1148, 620, "3. Download & Extract");
    auto *target_box = new Fl_Group(34, 145, 1110, 170);
    auto *target_title = new Fl_Box(52, 158, 300, 28, "Selected content");
    target_title->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE); target_title->labelfont(FL_BOLD); target_title->labelsize(18);
    auto *dlabel = new Fl_Box(52, 202, 90, 28, "Depot"); dlabel->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    auto *dview = new Fl_Box(142, 202, 250, 28, "Choose in Depot Browser"); dview->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    auto *vlabel = new Fl_Box(52, 242, 90, 28, "Version"); vlabel->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    auto *vview = new Fl_Box(142, 242, 250, 28, "Choose a version"); vview->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    auto *hint = new Fl_Box(430, 198, 650, 70,
        "Download stores the required DAT/blob data under the working directory.\nExtract reconstructs the depot into the output directory.");
    hint->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE | FL_ALIGN_WRAP); hint->labelsize(13);
    target_box->end();

    state.output_dir = new Fl_Input(52, 340, 900, 34);
    state.output_dir->value("steam2data/extracted");
    auto *out_label = new Fl_Box(52, 314, 160, 26, "Extraction output");
    ...
    state.download_button = new Fl_Button(52, 410, 240, 54, "Download Version"); state.download_button->labelsize(17);
    state.extract_button = new Fl_Button(304, 410, 240, 54, "Extract Version"); state.extract_button->labelsize(17);
    state.download_extract_button = new Fl_Button(556, 410, 300, 54, "Download + Extract"); state.download_extract_button->labelsize(17);
    auto *open_data = new Fl_Button(868, 410, 252, 54, "Open Data Folder");
    auto *action_help = new Fl_Box(52, 490, 1068, 60,
        "Tip: start from a depot in the browser. For a reset depot, select the appropriate blob CRC shown there before extracting.");
    ...
    default_btn->callback([](Fl_Widget *, void *d) {
        auto &state = *static_cast<State *>(d);
        state.mirror->value("https://de.steam2.download/");
    }, &state);
    browse_index->callback([](Fl_Widget *, void *d) { choose_file(static_cast<State *>(d)->index_file, "Choose existing index"); }, &state);
    browse_index_out->callback([](Fl_Widget *, void *d) { choose_file(static_cast<State *>(d)->index_file, "Choose index output"); }, &state);
    browse_data->callback([](Fl_Widget *, void *d) { choose_directory(static_cast<State *>(d)->data_dir, "Choose working directory"); }, &state);
    out_browse->callback([](Fl_Widget *, void *d) { choose_directory(static_cast<State *>(d)->output_dir, "Choose extraction output"); }, &state);
    state.index_button->callback(index_cb, &state);
    jump->callback([](Fl_Widget *, void *) { tabs->value(browser); }, nullptr);
    state.search->callback(search_cb, &state);
    refresh->callback([](Fl_Widget *, void *d) { rebuild_depot_list(*static_cast<State *>(d)); }, &state);
    state.depots_browser->callback(depot_select_cb, &state);
    state.versions_browser->callback(version_select_cb, &state);
    latest->callback(use_latest_cb, &state);
    details_btn->callback(copy_crc_cb, &state);
    ...
    open_data->callback([](Fl_Widget *, void *d) {
        auto &state = *static_cast<State *>(d);
#ifdef _WIN32
        const std::string command = "explorer " + quote_shell(value(state.data_dir));
#elif __APPLE__
        const std::string command = "open " + quote_shell(value(state.data_dir));
#else
        const std::string command = "xdg-open " + quote_shell(value(state.data_dir));
#endif
