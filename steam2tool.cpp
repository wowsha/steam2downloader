...
struct Entry { std::string filename; };
...
void import_index_text(std::string_view text, std::map<std::string, Depot>& depots, bool blob) {
    ...
    while (std::getline(in, line)) {
        ...
        while (iss >> token) {
            ...
            auto& d=depots.try_emplace(depot,Depot{depot}).first->second; (blob?d.blobs:d.dats).push_back({token}); break;
        }
    }
}
